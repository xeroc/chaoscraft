...
async function createIssueAndUpdateDb(
  paymentDbId: number,
  requestText: string,
  priority: string,
) {
  const { data: issue } = await octokit.rest.issues.create({
    ...
    labels: [
      "ready-for-build",
      priority === "standard" ? "" : `priority:${priority}`,
    ].filter(Boolean),
  });
  ...
  await run(
    `INSERT INTO requests (issue_number, request_text, priority) VALUES ($1, $2, $3)`,
...
export async function POST(req: NextRequest) {
  try {
    ...
    const payment = await queryOne("SELECT * FROM payments WHERE id = $1", [
      paymentDbId,
    ]);
    ...
    await run(
      `UPDATE payments SET status = $1, verified_at = $2, payment_id = $3 WHERE id = $4`,
      ["verified", new Date().toISOString(), signature, paymentDbId],
    );

    // Create GitHub issue
    const issue = await createIssueAndUpdateDb(
      paymentDbId,
      requestText,
      priority,
    );
    ...
    console.log(
      `✅ Solana payment verified and Issue #${issue.number} created for request: ${requestText}`,
    );
